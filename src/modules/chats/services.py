...
class ChatsService:
    ...
    async def get_chat_messages(chat_id: str):
        async with async_session() as session:
            messages_req = select(Message).where(Message.chat_id == chat_id).order_by(Message.created_at.desc())
    ...
    async def insert_message(chat_id: int, user_prompt: str) -> int:
        async with async_session() as session:
            async with session.begin():
                try:
                    insert_message_req = insert(
                        Message
                    ).values(
                        chat_id=chat_id, body=user_prompt,
    ...
    async def create_chat(user_prompt: str, user_id: int):
        async with async_session() as session:
            async with session.begin():
                insert_chat_req = insert(Chat).values(title=user_prompt[:50], user_id=user_id).returning(Chat.id)
